...
import type { LawyerSignUpRequest, SignUpResponse } from '../backend/schema';
import { useSignUpContext } from '../context/sign-up-context';

export function useLawyerSignUp() {
  const { dispatch } = useSignUpContext();

  return useMutation({
    mutationFn: async (data: Omit<LawyerSignUpRequest, 'type'>) => {
    ...
    onMutate: () => {
      dispatch({ type: 'SUBMIT_REQUEST', tab: 'lawyer' });
    },
    onSuccess: () => {
      dispatch({ type: 'SUBMIT_SUCCESS', tab: 'lawyer' });
    },
    onError: (error: any) => {
      const message =
