...
1. /docs/{requirement, persona, techstack, structure,prd,userflow,database,common-modules}.md, /docs/rules/tdd.md 문서를 읽고 프로젝트의 상태를 구체적으로 파악한다.
...
7. 완성된 설계를 `docs/pages/{N-name}/state.md` 적절한 N-name 경로를 생성하여 저장한다.
  - 모든 페이지에 대해서 적절한 이름으로 생성 및 state.md를 작성한다.
...
각각의 모든 특정 페이지에 대한 구체적인 상태관리설계 문서를 `docs/pages/{N-name}/state.md` 경로에 작성한다.